...
<div class="templates" style="display:none;">
  ...
  <div class="desc-template desc">[desc]</div>
...
<script>
  const populateDays = days => {
    ...
    const descTemplate = document.querySelector('.desc-template')
    Object.keys(days).sort().forEach(dayKey => {
      ...
      days[dayKey].forEach(event => {
        ...
        if (event.desc) {
          const descEl = descTemplate.cloneNode(true)
          descEl.classList.remove('desc-template')
          const lines = event.desc
            .split('\n')
            .map(line => line.trim())
            .filter(line => line[0] !== '!')
            .join('<br>')
